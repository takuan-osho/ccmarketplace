...
codex exec --model gpt-5.2-codex "Your prompt here"

# Using config override
codex exec --config model='"gpt-5.2-codex"' "Your prompt here"

# Multi-line prompt using heredoc
codex exec --model gpt-5.2-codex "$(cat <<'EOF'
...
cd /path/to/project && codex exec --model gpt-5.2-codex "Analyze this project"

# Reading file content into prompt
codex exec --model gpt-5.2-codex "Review this code: $(cat src/auth.ts)"
```
...
CODEX_RESULT=$(codex exec --model gpt-5.2-codex "prompt")
...
# Pass to agent
codex exec --model gpt-5.2-codex "Review this: $FILE_CONTENT"
```
...
STEP1=$(codex exec --model gpt-5.2-codex "First analysis")
...
codex exec --model gpt-5.2-codex "Analysis A" > /tmp/result_a.txt &
...
timeout 300 codex exec --model gpt-5.2-codex "Long running analysis"
```
...
# Simple retry
for i in 1 2 3; do
  result=$(codex exec --model gpt-5.2-codex "prompt") && break
  sleep $((i * 2))
done
```
